    ...
    if (!userStates[userId]) {
      ...
      switch (userStates[userId].state) {
        case "waiting_for_response":
          switch (lowerCaseMessage) {
            ...
            case "3":
              client
                .sendText(
                  userId,
                  "Por favor, envie uma imagem clara do bicho ou planta.📸🌿 Se quiser, pode incluir uma legenda na próxima mensagem."
                )
                .then(() => {
                  userStates[userId] = {
                    state: "waiting_for_species_image",
                    data: {},
                    timeout: setTimeout(() => {
                      if (userStates[userId]) {
                        client.sendText(
          ...
          break;
        case "waiting_for_species_image":
          if (message.type === "image") {
            try {
              const mediaData = await client.decryptFile(message);
              const filename = `especie_${userId}_${Date.now()}.jpg`;
              fs.writeFileSync(filename, mediaData);

              userStates[userId].data.filename = filename;

              client
                .sendText(
                  userId,
                  "Se quiser, envie uma breve legenda ou nome da espécie 🌿📝. Se não quiser, apenas escreva 'não'."
                )
                .then(() => {
                  userStates[userId].state = "waiting_for_species_caption";
                });
            } catch (error) {
              console.error("Erro ao salvar imagem de espécie:", error);
              client.sendText(
                userId,
            ...
            client.sendText(userId, "Por favor, envie uma imagem válida.📸🪲");
          }
          break;

        case "waiting_for_species_caption":
          const caption =
            lowerCaseMessage === "não" ? "Sem legenda fornecida" : message.body;

          const especieData = {
            userId: userId,
            caption: caption,
            filename: userStates[userId].data.filename,
          };

          appendToSpeciesExcel("especies.xlsx", especieData)
            .then(() => {
              client
                .sendText(
                  userId,
                  "Imagem do bicho ou planta recebida com sucesso!🌿📸 Obrigado por contribuir com o Observatório do Mangue!✅"
                )
                .then(() => {
                  delete userStates[userId];
                });
            ...
            .catch((error) => {
              console.error("Erro ao salvar no Excel de espécies:", error);
              client.sendText(userId, "Erro ao salvar os dados no Excel ❌.");
              delete userStates[userId];
            });
          break;
...
// Função de salvar a imagem do bicho ou planta
async function appendToSpeciesExcel(filePath, data) {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      worksheet = workbook.addWorksheet("Especies");
      worksheet.addRow(["UserID", "Legenda", "Imagem"]);
    }
